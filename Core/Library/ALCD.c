...
typedef enum {
	ALCD_RS_State_Command = 0, ALCD_RS_State_Data = 1,
} ALCD_RState;

typedef enum {

	SYS_OK = 0, SYS_Init = 1, SYS_LCD_ERR = 2, SYS_ADC_ERR = 3, SYS_END = 0xFF,
} SYS_ERR_e;
...
void ALCD_init(uint8_t col, uint8_t row) {
	...
	ALCD_write4Bit(ALCD_RS_State_Command,
	...
	ALCD_write(ALCD_RS_State_Command,
	ALCD_CMD_FUNCTION_SET | ALCD_FLAG_FUNCTION_SET_4BIT |
...
void ALCD_putNum(int32_t num) {
	char temp[13];
	snprintf(temp, sizeof(temp) - 1, "%ld", num);
	ALCD_puts(temp);
}
/**
